...
/** Import dropped .ics files into the primary writable calendar. */
export async function importIcsFiles(files: File[]): Promise<IcsImportResult> {
  ...
  for (const file of files) {
    ...
    for (const ev of parsed) {
      await createEvent(target.id, ev as Partial<GEvent> & { start: GDateTime; end: GDateTime })
      added++
    }
